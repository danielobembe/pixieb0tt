...
class API(object):
    ...
    def __init__(self, aus = False, ssl_prefix = '', locale = ''):
        ...
        """
        self.convert_price = convert_price # alias for price_mod function
        self.get_price_spread = get_price_spread # alias for price_mod function
        self.abs_path = os.path.abspath(os.path.dirname(__file__))
        self.certs_paths = self.load_ssl_cert_paths(ssl_prefix)
        self.aus = aus
        self.app_key = '' # Use create_app_keys() or see online api docs.
        self.locale = locale
        self.session_token = ''

    def load_ssl_cert_paths(self, ssl_prefix = ''):
        ...
        if not ssl_prefix:
            ...
            raise Exception(msg)
        cert_paths = []
        ssl_path = self.abs_path + '/ssl_certs/'
    ...
    def get_settled_bets(self, market_id = '', group_by = 'MARKET', req_id = 1):
    ...
    def get_current_bets(self, market_ids = None, req_id = 1):
        ...
        if type(resp) is dict and 'result' in resp:
            # parse bets
            bets = {}
            if 'currentOrders' in resp['result']:
                for order in resp['result']['currentOrders']:
                    key = order['marketId']
                    if key not in bets: bets[key] = []
                    bets[key].append(order)
            return bets
        else:
            raise Exception(str(resp))
